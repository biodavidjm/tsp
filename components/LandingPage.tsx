...
export default function Component() {
  ...
  const radius = Math.min(dimensions.width, dimensions.height) * 0.25 // Reduced radius for shorter connections
    ...
    <div className="relative w-full h-screen bg-gradient-to-br from-gray-900 to-gray-800 overflow-hidden">
      ...
      <svg width={dimensions.width} height={dimensions.height} className="absolute inset-0">
        ...
        {entities.map((entity, index) => {
          ...
          const textX = centerX + (radius + 30) * Math.cos(angle) // Moved text further out
          const textY = centerY + (radius + 30) * Math.sin(angle) // Moved text further out
          return (
            <g key={entity}>
              <motion.line
                x1={centerX}
